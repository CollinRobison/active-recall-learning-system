...
**Implementation status:** Protocol, schemas, portable skills, runtime workspace tools, optional embeddings/Milvus boundary, grounded model adapters, durable tutor/session orchestration, catalog/paths, adaptive review, and ingestion hardening are implemented. Deterministic golden-learning, exact citation-location, local provider-contract, and harness-installation coverage is implemented. Live third-party harness and installed-Milvus compatibility validation remain external-environment work.
...
5. **Evaluation and adapters:** completed deterministic golden learning examples/evaluation cases, exact citation-location tests, local command/OpenAI-compatible provider integration tests, and concrete installation/invocation instructions for every listed harness adapter. Live vendor-harness and live provider acceptance runs remain external-environment work.

The catalog runtime now creates, edits (with explicit preview/confirmation), and validates Markdown topic/path records; maintains reciprocal source associations; rejects prerequisite cycles; and produces a transparent ranking of open confusion, unblocked active path order, and active topics. Its prerequisite gate uses explicit completion or persisted correct evidence without unresolved confusion. Adaptive review now maintains one current due entry per topic plus append-only history, records recall/explanation/application evidence, delayed-review status, and confidence calibration, and supplies recent concepts to question generation for interleaving. Ingestion now supports safe opt-in repository/code traversal, optional local image OCR with explicit degraded/required modes, metadata provenance and confirmation, and duplicate-source conflict records. Citation validation now verifies exact headings, page markers, line ranges, URL locations, or exact extracted excerpts rather than merely accepting a location label. Milvus has generation-safe full rebuild publication, incremental updates, and source/topic/path query filters; deterministic fake-client tests cover that boundary without requiring pymilvus. The evaluation suite includes hand-authored correct, partial, incorrect, overconfident, alternative-wording, conflicting-source, and missing-context cases, plus a local OpenAI-compatible HTTP integration test. PDF page rendering OCR, richer webpage parsing, installed-Milvus compatibility validation, and live third-party harness/provider acceptance remain external-environment work.
