...
function overview(){const s=DATA.summary,p=DATA.progress; $('#overview').innerHTML=`<div class="metrics">${metric('Completion',s.topic_completion_percentage+'%','topics','completed')}${metric('Topics',s.topics,'topics')}${metric('Questions answered',s.questions,'sessions')}${metric('Open confusion',s.open_confusion,'confusion','open')}</div><div class="split"><div class="panel"><h3>Learning evidence</h3>${bars(p.evaluations)}<div class="actions"><button class="action" onclick="goFiltered('confusion','open')">Review ${s.open_confusion} open confusion ${plural(s.open_confusion,'item')}</button><button class="action" onclick="goFiltered('topics','active')">Browse ${s.active_topics} active topics</button></div></div><div class="panel"><h3>Topic completion</h3><div class="completion"><div class="ring" style="--pct:${s.topic_completion_percentage}"><b>${s.topic_completion_percentage}%</b></div><div><b>${s.completed_topics} of ${s.topics} topics complete</b><p class="meta">Completion reflects explicit topic status, not an inferred mastery score.</p><button class="action" onclick="goFiltered('topics','completed')">Inspect completed topics</button></div></div></div></div><div class="split"><div class="panel"><h3>Path completion</h3>${pathBars(DATA.path_completion)}</div><div class="panel"><h3>Study activity</h3>${activityChart(DATA.session_activity)}</div></div><div class="panel"><h3>Quick read</h3><div class="notice">${s.sources?`This workspace contains ${s.sources} ${plural(s.sources,'source')}, ${s.topics} ${plural(s.topics,'topic')}, and ${s.paths} ${plural(s.paths,'path')}.`:'Ingest a source, then create topics and a learning path to populate this dashboard.'}</div>${bars(DATA.status_counts.sources)}</div>`; $('#overview').classList.remove('hidden'); $('#records').classList.add('hidden')}
