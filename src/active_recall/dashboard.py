...
function status(v){return `<span class="status ${esc(v)}">${esc(nice(v||'unknown'))}</span>`} function links(ids,map,target){return (Array.isArray(ids)?ids:[]).map(id=>`<button class="tag tag-link" data-link-kind="${esc(target)}" data-link-id="${esc(id)}">${esc(map[id]||id)}</button>`).join('')} function provenance(items){return (Array.isArray(items)&&items.length)?`<div class="provenance">Extracted with: ${items.map(x=>`<code>${esc(x)}</code>`).join(', ')}</div>`:''}
function metric(label,value,target,filter='all'){return `<button class="metric metric-button" onclick="goFiltered('${target}','${filter}')"><div class="label">${label}</div><div class="value">${value}</div></button>`} function bars(counts){const entries=Object.entries(counts||{});const max=Math.max(1,...entries.map(x=>x[1]));return entries.length?entries.map(([n,v])=>`<div class="bar-row"><span>${esc(nice(n))}</span><div class="bar"><i style="width:${v/max*100}%"></i></div><b>${v}</b></div>`).join(''):'<div class="empty">No evidence yet.</div>'} function sourceHealth(counts){const entries=Object.entries(counts||{});if(!entries.some(([name])=>name!=='complete'))return '';return `<div class="source-health"><h3>Source extraction status</h3><div class="notice">Some content sources were not extracted completely. Review their Content records before studying from them.</div>${bars(counts)}</div>`} function pathBars(paths){return paths.length?paths.map(x=>`<div class="path-row"><button onclick="go('paths')">${esc(x.name)}</button><div class="bar"><i style="width:${x.percentage}%"></i></div><b>${x.percentage}%</b></div>`).join(''):'<div class="empty">Create a learning path to see per-path completion.</div>'} function activityChart(activity){const entries=Object.entries(activity||{}).slice(-14);if(!entries.length)return '<div class="empty">Study sessions will appear here as activity is recorded.</div>';const max=Math.max(...entries.map(x=>x[1]));return `<div class="spark">${entries.map(([d,v])=>`<i title="${d}: ${v} questions" style="height:${Math.max(8,v/max*100)}%"></i>`).join('')}</div><div class="spark-labels"><span>${esc(entries[0][0])}</span><span>${esc(entries.at(-1)[0])}</span></div>`}
let chartScope='all';
