...
def record_review(root: Path, *, topic_id: str, session_id: str, classification: str, confidence: int | None, next_review_at: str, citation: str) -> dict[str, Any]:
    root = root.expanduser().resolve(); due = root / "reviews" / "due.md"; history = root / "reviews" / "history.md"
    key = f"{topic_id}|{session_id}"
    entry = {"topic_id": topic_id, "session_id": session_id, "classification": classification, "confidence": confidence, "next_review_at": next_review_at, "citation": citation, "updated_at": now_iso()}
    ...
    lines.append(f"- `{key}` — due {next_review_at}; result={classification}; confidence={confidence if confidence is not None else 'unrecorded'}; citation={citation or 'unavailable'}")
    atomic_write(due, "\n".join(lines).rstrip() + "\n")
