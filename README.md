...
9. Build the optional vector index: `python -m pip install -e '.[milvus,local-embeddings]'`, then `learning vector-reindex ~/Learning --embedding-provider sentence-transformers`. Full rebuilds publish a new generation only after it is built; use `--incremental` to upsert changed chunks in the current published generation. Tutor commands use `--retrieval-engine auto` by default: they prefer a compatible current Milvus index when one exists, then fall back to the lexical manifest if vectors are unavailable, stale, mismatched, fail, or return no evidence.
