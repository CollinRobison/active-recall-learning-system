...
9. Preview removal of one source, topic, or path: `learning remove ~/Learning RECORD_ID`; review the relationship-aware plan, then permanently apply it only with `--confirm`. It keeps a path that still has other content, removes only empty paths, rebuilds the lexical index, clears vector cache, and refreshes the dashboard.
10. Generate a local, queryable snapshot dashboard: `learning dashboard ~/Learning` and open `~/Learning/dashboard.html` directly in a browser. It is read-only derived output and is automatically refreshed after successful CLI data mutations; refresh it manually after direct external edits.
11. Build the optional vector index: `python -m pip install -e '.[milvus,local-embeddings]'`, then `learning vector-reindex ~/Learning --embedding-provider sentence-transformers`. Full rebuilds publish a new generation only after it is built; use `--incremental` to upsert changed chunks in the current published generation. Tutor commands use `--retrieval-engine auto` by default: they prefer a compatible current Milvus index when one exists, then fall back to the lexical manifest if vectors are unavailable, stale, mismatched, fail, or return no evidence.
12. Generate/evaluate model-driven turns through a local JSON adapter:
...
- **Skills:** [active recall](skills/active-recall.md), [application practice](skills/application-practice.md), [catalog](skills/catalog.md), [confusion review](skills/confusion-review.md), [Feynman teachback](skills/feynman-teachback.md), [index maintenance](skills/index-maintenance.md), [learning dashboard](skills/learning-dashboard.md), [remove learning records](skills/remove-learning-records.md), [progress](skills/progress.md), [source ingest](skills/source-ingest.md), [study session](skills/study-session.md), [teach](skills/teach.md), [workspace init](skills/workspace-init.md)
